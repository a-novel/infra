...
variable "backup_uploader_image" {
  ...
  default = "docker.io/alpine/curl:8.22.0"

  validation {
    condition     = can(regex("^docker\\.io/alpine/curl:(0|[1-9][0-9]*)\\.(0|[1-9][0-9]*)\\.(0|[1-9][0-9]*)$", var.backup_uploader_image))
    error_message = "The curl image must use a complete stable SemVer tag."
  }
}
