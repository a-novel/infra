...
services and jobs through a deployment-only custom role, manage Cloud Scheduler entries, attach the
six foundation-owned non-database runtime identities, and set IAM policies on release-owned services
and jobs through that same exact role. The deployment role excludes `run.jobs.run` and
`run.jobs.runWithOverrides`; exact job-level `roles/run.invoker` bindings grant routine execution. It
cannot change project IAM, VPC policy, remote-state protection, the preserved database disk, snapshot
lifecycle, backup retention, or secret payloads. Cloud Run resolves exact numeric secret versions as
the dedicated runtime identity, so release automation never reads a credential.
...
gRPC service, its exact invoker allowlist, and the public Authentication REST service. Key rotation
runs once per deployment and every hour. Only named human principals may execute initialization; it
...
`authentication_initializer_principals` accepts only named `user:` or `group:` IAM members and must
contain at least one entry before the application is enabled. Those principals receive
`roles/run.invoker` on `agora-authentication-init` only. The initializer uses its own service identity
to read the Authentication DSN and bootstrap password; the REST identity cannot read that password.
...
- exact foundation-owned Authentication, Authentication-initializer, JSON Keys, backup, restore, and
  scheduler service-account emails;
- at least one named human `authentication_initializer_principals` member when the application is
  enabled;
