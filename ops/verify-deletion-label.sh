...
# Verifies human-maintainer approval now (PR number) or at merge (exact commit).
# The caller checks current PR state; this script replays its label history.
# Usage: verify-deletion-label.sh <owner/repository> <commit|pr-number>

set -euo pipefail

if [ "$#" -ne 2 ]; then
    printf 'Usage: %s <owner/repository> <commit|pr-number>\n' "$0" >&2
    exit 64
fi
...
if ! APPROVER="$(jq --exit-status --raw-output \
    ...
    --arg label "${REQUIRED_LABEL}" '
      reduce (
        ...
        {merged: false, approved: false, actor: null, via_app: null};
        if .merged then .
        elif $event.event == "labeled" and $event.label.name == $label then
          .approved = true | .actor = $event.actor | .via_app = $event.performed_via_github_app
        elif $event.event == "unlabeled" and $event.label.name == $label then
          .approved = false | .actor = null
        elif $event.event == "merged" then
          .merged = true
        else . end
      )
      | select(.merged == ($current | not) and .approved and
          .via_app == null and .actor.type == "User" and
